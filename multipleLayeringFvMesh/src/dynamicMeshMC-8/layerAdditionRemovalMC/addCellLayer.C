...
Foam::tmp<Foam::vectorField> Foam::layerAdditionRemovalMC::extrusionDir() const
{
    ...
    else
    {
        ...
        extrusionDir = (oldLayerThickness_*removeFactor_)*masterFaceLayer.pointNormals();
    }
    return textrusionDir;
}


void Foam::layerAdditionRemovalMC::addCellLayer
(
    polyTopoChange& ref
) const
{
    ...
    if (debug)
    {
        Pout<< "void layerAdditionRemovalMC::addCellLayer("
    ...
    const polyMesh& mesh = topoChanger().mesh();
    const primitiveFacePatch& masterFaceLayer = mesh.faceZones()[faceZoneID_.index()]();
    ...
    // Added points field for indentifying transversal face direction
    pointField addedPointsField(mp.size());
    ...
    forAll(mp, pointI)
    {
        ...
        addedPoints[pointI] =
            ...
            (
                ...
                (
                  ...
                  + addLayerFactor_*tpointOffsets()[pointI],
        ...
        // Grab new point coordinates for future detecting new face orientation
        addedPointsField[pointI] = points[mp[pointI]] + addDelta_*tpointOffsets()[pointI];
    }
    ...
    for (label curEdgeID = 0; curEdgeID < nInternalEdges; curEdgeID++)
    {
        ...
        // Se cargan las coordenadas de los puntos respectivos de las caras
        newFacePoints[0] = points[mp[zoneLocalEdges[curEdgeID].start()]];
        newFacePoints[1] = points[mp[zoneLocalEdges[curEdgeID].end()]];
        newFacePoints[2] = addedPointsField[zoneLocalEdges[curEdgeID].end()];
        newFacePoints[3] = addedPointsField[zoneLocalEdges[curEdgeID].start()];


/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        pointField pointsNewPoints(addedPoints.size() + ref.points().size()); //Puntos viejos + nuevos

        for(label pointI = 0; pointI < 4; pointI++) // Cargar las coordenadas de los puntos de la cara nueva en la lista de todos los puntos
        {
            pointsNewPoints[newFace[pointI]] = newFacePoints[pointI];
        }

        const vector newFaceSf = newFace.normal(pointsNewPoints);

        // Bucle por todas las caras del corriente edge!
        forAll(curFaces, faceI)
        {
            const label cf = curFaces[faceI];
            label znID = zoneMesh.whichZone(cf);
            bool zoneFaceFlip;

            if (znID != faceZoneID_.index() && znID != -1)
            {
                zoneID = mesh.faceZones().whichZone(cf);
                const label& initialZoneFaceLabel = mesh.faceZones()[zoneID][0];
                zoneFaceFlip = mesh.faceZones()[zoneID].flipMap()[0];

                pointField initialZoneFacePoints(faces[initialZoneFaceLabel].size());

                forAll(initialZoneFacePoints, pointI)
                {
                    initialZoneFacePoints[pointI] = mesh.points()[faces[initialZoneFaceLabel][pointI]];
                }

                const vector initialZoneFaceSf = faces[initialZoneFaceLabel].normal(points);

                Info << "Vector de cara a agregar: " << newFaceSf << " Vector de faceZone: " << initialZoneFaceSf << endl;

                scalar direction = initialZoneFaceSf & newFaceSf;

                if ((direction > 0 && zoneFaceFlip == false) || (direction < 0 && zoneFaceFlip == true))
                {
                    faceFlip = false;
                }
                else
                ...
                //faceFlip = mesh.faceZones()[zoneID].flipMap()[0];
            }
    ...
    )
    {
        ...
        const labelList& curFaces = meshEdgeFaces[meshEdges[curEdgeID]];
        //Pout <<"Llegue antes de crear patchIds" << endl;
        List<bool> patchIds(mesh.boundaryMesh().names().size(), false);
                ...
                if (znID != faceZoneID_.index())
                {
                    ...
                    **/
                    ...
                    if (zoneID > -1)
                    {
                        ...
                        //Pout << "** " << zoneFaceFlip << nl;

                        const label& initialBoundaryFaceLabel = mesh.faceZones()[zoneID][0];
                        const vector initialBoundaryFaceSf = faces[initialBoundaryFaceLabel].normal(points);

                        scalar direction = initialBoundaryFaceSf & newFaceSf;
        ...
        if (patchID < 0)
        {
            FatalErrorIn
            (
                "void Foam::layerAdditionRemovalMC::setRefinement("
                "polyTopoChange& ref)"
