...
bool Foam::layerAdditionRemovalMC::setLayerPairing() const
{
    // Note:
    ...
    if (pointsPairingPtr_ || facesPairingPtr_)
    {
        FatalErrorIn
        (
            "void Foam::layerAdditionRemovalMC::setLayerPairing() const"
...
//         Pout<< "ptc: " << ptc << endl;
    ...
    reduce(nPointErrors, sumOp<label>(), 0, commIndex_);
    reduce(nFaceErrors, sumOp<label>(), 0, commIndex_);
...
const Foam::labelList& Foam::layerAdditionRemovalMC::pointsPairing() const
{
    if (!pointsPairingPtr_)
    {
        FatalErrorIn
        (
            "const labelList& layerAdditionRemovalMC::pointsPairing() const"
...
const Foam::labelList& Foam::layerAdditionRemovalMC::facesPairing() const
{
    if (!facesPairingPtr_)
    {
        FatalErrorIn
        (
            "const labelList& layerAdditionRemovalMC::facesPairing() const"
        )   << "Problem with layer pairing data for object " << name()
            << abort(FatalError);
    }
...
void Foam::layerAdditionRemovalMC::modifyMotionPoints
(
...
) const
{
    if (debug)
    {
        Pout<< "void layerAdditionRemovalMC::modifyMotionPoints("
