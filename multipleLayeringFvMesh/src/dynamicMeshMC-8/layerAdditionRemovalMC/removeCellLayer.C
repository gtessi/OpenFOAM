...
bool Foam::layerAdditionRemovalMC::validCollapse() const
{
    ...
    if (returnReduce(nBoundaryHits, sumOp<label>(), 0, commIndex_))
    {
        return false;
    }
    else
    ...
    label globalnBoundaryHits = nBoundaryHits;

    reduce(globalnBoundaryHits, sumOp<label>(), 0, commIndex_);

    return true;

    if (globalnBoundaryHits > 0)
    {
        return false;
    }
    else
...
void Foam::layerAdditionRemovalMC::removeCellLayer
(
    polyTopoChange& ref
) const
{
